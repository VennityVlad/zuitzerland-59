...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 1: Remove user from allowlist if present
    ...
    if (allowlistData.entries) {
      allowlistEntry = allowlistData.entries.find((entry: any) => 
    ...
    // Step 2: Find and revoke user sessions
    // Get user details
    ...
    // Step 3: Revoke all sessions for this user
    const revokeSessionsUrl = `https://auth.privy.io/api/v1/users/${userId}/sessions/revoke`;
    const revokeResponse = await fetch(revokeSessionsUrl, {
      method: 'POST',
      headers: {
        'Authorization': `Basic ${base64Auth}`,
        'privy-app-id': appId,
    ...
    if (!revokeResponse.ok) {
      ...
      console.error('Failed to revoke sessions:', errorText);
    ...
    return new Response(
      JSON.stringify({ 
        ...
        details: {
          userId,
          removedFromAllowlist: !!allowlistEntry
        }
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );
